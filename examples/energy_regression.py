...
###########################################
# Predictions on test sample and plotting #
###########################################
...
def print_pcolor_labels(ax, X, Y, C):
    for x1, x2, y1, y2, c in zip(
        X[:-1, :-1].flatten(), X[1:, 1:].flatten(), Y[:-1, :-1].flatten(), Y[1:, 1:].flatten(), C.flatten()
    ):
        x = np.mean([x1, x2])
        y = np.mean([y1, y2])
        if not np.isnan(c):
...
df_test = load_data(file_name.replace("training", "testing"), isEE=False, entrystop=None)
...
median, x_edges, y_edges, binnumber = stats.binned_statistic_2d(
    df_test["eta"], df_test["pt"], z_bo, bins=[bins_eta, bins_pt], statistic="median"
)
effrms, x_edges, y_edges, binnumber = stats.binned_statistic_2d(
    df_test["eta"], df_test["pt"], z_bo, bins=[bins_eta, bins_pt], statistic=rmseff
)
